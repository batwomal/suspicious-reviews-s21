...
def count_schema_types(schema: GraphQLSchema) -> Dict[str, int]:
    """
    ...
    queries = get_root_operation_types(schema, 'query')
    mutations = get_root_operation_types(schema, 'mutation')
    subscriptions = get_root_operation_types(schema, 'subscription')
...
def get_root_operation_types(schema: GraphQLSchema, operation_type: str) -> List[str]:
    """
...
def get_operation_definitions(schema: GraphQLSchema, operation_types: List[str]) -> List[Dict]:
    """
...
def get_special_types(schema: GraphQLSchema, type_prefix: str) -> Dict[str, GraphQLType]:
    mapping = {
    ...
    return {k: v for k, v in schema.type_map.items() if isinstance(v, mapping[type_prefix])}
...
def create_enums_ddl(schema: GraphQLSchema) -> str:
    enums_ddl = []
